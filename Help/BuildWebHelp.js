...
var xslTopicFile = "";
xslTopicFile += fileSystem.GetFolder(".\\XSL\\") + "\\topic.xsl";
...
var xslOverviewFile = "";
xslOverviewFile += fileSystem.GetFolder(".\\XSL\\") + "\\overview.xsl";
...
while (!fileEnum.atEnd())
{
	...
	var lnkFileNameFull   = "Lnk" + htmlFileNameFull;
	var destinationFolder = fileSystem.GetFolder(".\\WebHelp\\");
	
	htmlFileNameFull = destinationFolder + "\\" + htmlFileNameFull;
	lnkFileNameFull  = destinationFolder + "\\" + lnkFileNameFull;
	...
	try
	{
		...
		// Create link file.
		if (!fileSystem.FileExists(lnkFileNameFull))
		{
			var lnkFile     = fileSystem.CreateTextFile(lnkFileNameFull, false);
			var lnkText     = '<html><head><title>Open Twebst | Help - IE macro recorder for web automation and web testing: ' + baseFileName + '</title>' + '</head><frameset cols="22%, *" frameborder="yes"><frame src="Toc.htm"><frame src="' + htmlFileName + '" name="topic_pane"></frameset></html>';

			lnkFile.Write(lnkText);
			lnkFile.Close();
		}
	}
	catch(err)
